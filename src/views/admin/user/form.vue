    ...
    <el-dialog :visible.sync="dialog_visible" :title="title" :close-on-click-modal="false" class="activation_dialog" >
      <el-form v-loading="loading" ref="form" :rules="rules" :model="form" label-width="120px" >
        <el-form-item label="管理员账号" prop="id">
          <el-input v-model="form.id" placeholder="请输入管理员id" auto-complete="off"/>
        </el-form-item>
        <div>
          <el-form-item label="密码" prop="password">
            <el-input v-model="form.password" placeholder="请输入管理员账号密码" />
          </el-form-item>
          <el-form-item label="确认密码" prop="c_password">
            <el-input v-model="form.c_password" placeholder="请输确认密码" type="password" auto-complete="off"/>
          </el-form-item>
        </div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        phone: [
          { message: '手机号码格式不正确', pattern: /^((0\d{2,3}-\d{7,8})|(1[34578]\d{9}))$/, trigger: 'change' }
        ],
        password: [
        ...
        c_password: [
          { required: true, validator: this.Pwd, trigger: 'blur' }
        ]
      },
      loading: false,
      router_auth: []
  ...
  created() {
    ...
    if (!this.id) {
      ...
      this.rules.c_password = [
        { required: true, trigger: 'blur', validator: validate_cpassword }
      ]
    }
  },
  methods: {
